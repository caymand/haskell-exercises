...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify s'
  where s' = palindromify' s

palindromify' s = substring 1 (length s - 1) s
...
eitherDiv x y = case safeDiv x y of Just num -> Right num
                                    Nothing -> Left (show x ++ "/0")
