...
sorted :: [Int] -> Bool
sorted xs = sorted' xs
    where sorted' (x1:x2:xs) = x1 <= x2 && sorted' (x2:xs)
          sorted' [x] = True
          sorted' [] = True
...
sumsOf (x:xs) = sumsOf' xs x
    where sumsOf' (x:xs) acc = acc : sumsOf' xs (acc+x)
          sumsOf' [] acc = [acc]
...
maybeMap f [] = []
maybeMap f (x:xs) = case f x of Just v -> v : maybeMap f xs
